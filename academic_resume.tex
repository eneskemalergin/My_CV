...
\begin{list2}
\item Search on the question; {\bf do chromatin state has any influence in alternative splicing?}
\item Developed a {\bf deep convolutional neural betwork} to predict any pattern from RNA-seq data of 11 different chromatin markers of HeLa cells.

\end{list2}
\item Research: Predicting transcription factor binding sites across cell types
\begin{list2}
\item Worked on a general tool that can {\bf predict transcription factor binding sites accross cell types.}
\item Dealt with {\bf large dataset} of approximately 900GB of data using linux tools and Python's data analysis suit.
\end{list2}
\end{list2}
...
Worked on drug development process and experimented on drug testing part. Was responsible for preparing agar solutions, bacteria cultures, and liquid/solid drug tests on those bacteria cultures. 
